    ...
    public class AuthenticateController : ControllerBase
    {
        ...
        public async Task<IActionResult> Login([FromBody] LoginModel model)
        {
            ...
            if (user != null && await _userManager.CheckPasswordAsync(user, model.Password))
            {
                ...
                {
                    return BadRequest("Invalid refresh token validity configuration");
                }
            }
            return Unauthorized();
        }
        ...
        public async Task<IActionResult> RegisterAdmin([FromBody] RegisterModel model)
        {
            ...
            ApplicationUser user = new()
            {
                ...
                SecurityStamp = Guid.NewGuid().ToString(),
                UserName = model.Username,
                RefreshToken = GenerateRefreshToken() // Set a non-null value for RefreshToken
            };
            var result = await _userManager.CreateAsync(user, model.Password);
            if (!result.Succeeded)
                return StatusCode(StatusCodes.Status500InternalServerError, new Response { Status = "Error", Message = "User creation failed! Please check user details and try again." });
        ...
        private JwtSecurityToken CreateToken(List<Claim> authClaims)
        {
            ...
            {
                throw new InvalidOperationException("Invalid token validity configuration");
            }
        }
